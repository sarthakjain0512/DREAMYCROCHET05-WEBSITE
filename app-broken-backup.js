...
document.addEventListener('DOMContentLoaded', () => {
  // --- ROBUST PRODUCT IMAGE SELECTION & FALLBACK HELPERS ---
  function extractImageUrl(val) {
    ...
    if (typeof val === 'object' && val !== null) {
      if (val.url && typeof val.url === 'string' && val.url.trim() !== '' && val.url !== '[object Object]') {
        return val.url.trim();
      }
      if (val.secure_url && typeof val.secure_url === 'string' && val.secure_url.trim() !== '' && val.secure_url !== '[object Object]') {
        return val.secure_url.trim();
      }
    }
    return null;
  }
  ...
  function getProductImageUrl(p) {
    if (!p) return '/images/product-placeholder.webp';

    // Priority 1: coverImage (preferred)
    let url = extractImageUrl(p.coverImage);
    if (url) return url;

    // Priority 2: image
    url = extractImageUrl(p.image);
    if (url) return url;

    // Priority 3: img
    url = extractImageUrl(p.img);
    if (url) return url;

    // Priority 4: First valid image in images[] gallery array
    if (Array.isArray(p.images) && p.images.length > 0) {
      for (const item of p.images) {
        const itemUrl = extractImageUrl(item);
        if (itemUrl) return itemUrl;
      }
    }

    // Priority 5: Fallback placeholder ONLY if absolutely no valid image exists
  ...
  function getAllProductImages(p) {
    ...
    if (Array.isArray(p.images)) {
      p.images.forEach(item => add(item));
    }
  ...
  const renderWishlistModalItems = () => {
    ...
    wishlist.forEach(prodName => {
      ...
      if (p) {
        ...
        const wishImgSrc = getProductImageUrl(p);
        itemEl.innerHTML = `
          <img src="${wishImgSrc}" alt="${p.name}" class="w-16 h-16 object-contain bg-beige/30 rounded-xl border border-primary/5 shrink-0" onerror="this.onerror=null; this.src='/images/product-placeholder.webp';">
  ...
  function renderProductCard(p, isAdmin) {
    ...
    if (isCustomOrderCard) {
      ...
      const mainImgSrc = getProductImageUrl(p);
      const allImgs = getAllProductImages(p).filter(u => !u.includes('product-placeholder'));
      const fallbackJson = JSON.stringify(allImgs).replace(/"/g, '&quot;');
      const onErrorAttr = allImgs.length > 1
        ? `(function(img){ if(!img.dataset.tIdx) img.dataset.tIdx='0'; var idx=parseInt(img.dataset.tIdx)+1; var arr=${fallbackJson}; if(idx<arr.length){ img.dataset.tIdx=idx; img.src=arr[idx]; }else{ img.onerror=null; img.src='/images/product-placeholder.webp'; if(img.parentElement) img.parentElement.classList.add('loaded'); } })(this);`
        : `this.onerror=null; this.src='/images/product-placeholder.webp'; if(this.parentElement) this.parentElement.classList.add('loaded');`;

      imgHtml = `<img src="${mainImgSrc}" alt="${p.name}" class="w-full h-full object-contain bg-beige/30 group-hover:scale-105 duration-500" loading="lazy" onload="this.parentElement.classList.add('loaded')" onerror="${onErrorAttr}">`;
    }
    ...
    const galleryImages = getAllProductImages(p);
    const showCollectionBtn = !isCustomOrderCard && galleryImages.length > 0 && !galleryImages[0].includes('product-placeholder');
  ...
  function renderAdminProductsTable(products) {
    ...
    products.forEach(p => {
      ...
      const adminThumbSrc = getProductImageUrl(p);
      row.innerHTML = `
        <td class="p-4 pl-6 flex items-center gap-3 font-semibold text-darkbrown">
          <img src="${adminThumbSrc}" alt="${p.name}" class="w-10 h-10 object-cover rounded-xl border border-primary/10" onerror="this.onerror=null; this.src='/images/product-placeholder.webp';">
...
console.log("images:", inquiryImageFiles);
  ...
  function openQuickView(p) {
    ...
    // Price formatting: always include currency symbol (₹)
    let priceStr = String(p.price || '');
    if (priceStr && !priceStr.startsWith('₹') && !priceStr.toLowerCase().includes('ask') && !priceStr.toLowerCase().includes('from')) {
      const numericVal = priceStr.replace(/^[^\d.]*/, '');
      priceStr = numericVal ? `₹${numericVal}` : priceStr;
    }
    qvPrice.textContent = priceStr || '₹0.00';

    // Populate description exclusively inside the expandable Product Details accordion panel
    if (qvFullDesc) {
      qvFullDesc.textContent = p.desc || 'Detailed handmade crochet craftsmanship created stitch by stitch.';
    }

    // Reset Accordion state to collapsed on open
    if (qvDetailsAccordion && qvDetailsToggleBtn) {
      ...
      qvDetailsToggleBtn.onclick = (e) => {
        e.stopPropagation();
        const isExpanded = qvDetailsAccordion.classList.toggle('expanded');
        qvDetailsToggleBtn.setAttribute('aria-expanded', isExpanded ? 'true' : 'false');
      };
    }

    qvType.textContent = p.badge || 'Handmade';
    
    // Bind wishlist button inside quick view modal
    if (qvWishlistBtn) {
      const isFav = typeof wishlist !== 'undefined' && wishlist.includes(p.name);
      qvWishlistBtn.querySelector('span').style.fontVariationSettings = isFav ? "'FILL' 1" : "'FILL' 0";
      qvWishlistBtn.onclick = (e) => {
        e.stopPropagation();
        if (typeof toggleWishlist === 'function') {
          toggleWishlist(p.name);
          const nowFav = wishlist.includes(p.name);
          qvWishlistBtn.querySelector('span').style.fontVariationSettings = nowFav ? "'FILL' 1" : "'FILL' 0";
        }
      };
    }

    // Normalize cover image URL
    const qvImgSrc = getProductImageUrl(p);
    qvImg.src = qvImgSrc;
  ...
  // --- HIGH-PERFORMANCE INTERACTIVE IMAGE ZOOM CONTROLLER ---
  let currentScale = 1;
  let currentTx = 0;
  let currentTy = 0;
  ...
  function applyZoomTransform(smooth = false) {
    ...
    if (smooth) {
      galleryMainImg.style.transition = 'transform 0.3s cubic-bezier(0.16, 1, 0.3, 1)';
    } else {
      galleryMainImg.style.transition = 'none';
    }
    galleryMainImg.style.transform = `translate3d(${currentTx}px, ${currentTy}px, 0) scale(${currentScale})`;
  }

  function clampZoomBounds() {
    if (!galleryMainImg || !galleryImageContainer) return;
    const containerRect = galleryImageContainer.getBoundingClientRect();
    const imgRect = galleryMainImg.getBoundingClientRect();

    const maxTx = Math.max(0, (imgRect.width * currentScale - containerRect.width) / 2);
    const maxTy = Math.max(0, (imgRect.height * currentScale - containerRect.height) / 2);

    if (currentScale <= 1.05) {
      currentTx = 0;
      currentTy = 0;
    } else {
      currentTx = Math.min(Math.max(currentTx, -maxTx), maxTx);
      currentTy = Math.min(Math.max(currentTy, -maxTy), maxTy);
    }
  }
  ...
  function initZoomGestureHandlers() {
    if (!galleryMainImg || !galleryImageContainer) return;

    // Touch events for mobile (Pinch to Zoom, Double Tap, Drag while Zoomed)
    galleryImageContainer.addEventListener('touchstart', (e) => {
      if (e.touches.length === 2) {
        ...
        if (currentScale > 1.05) {
          isZoomDragging = true;
          dragStartPos = {
            x: e.touches[0].clientX - currentTx,
            y: e.touches[0].clientY - currentTy
          };
        }
      }
    }, { passive: false });
    ...
    galleryImageContainer.addEventListener('touchmove', (e) => {
      if (e.touches.length === 2 && initialPinchDist > 0) {
        ...
        currentScale = Math.min(Math.max(1.0, initialScale * scaleRatio), 3.0);
        clampZoomBounds();
        applyZoomTransform(false);
      } else if (e.touches.length === 1 && isZoomDragging && currentScale > 1.05) {
        e.preventDefault();
        currentTx = e.touches[0].clientX - dragStartPos.x;
        currentTy = e.touches[0].clientY - dragStartPos.y;
        clampZoomBounds();
        applyZoomTransform(false);
      }
    }, { passive: false });
    ...
    // Desktop Mouse Events (Double Click & Drag)
    galleryMainImg.addEventListener('dblclick', (e) => {
      e.preventDefault();
      if (currentScale > 1.2) {
        resetImageZoom();
      } else {
    ...
    galleryImageContainer.addEventListener('mousedown', (e) => {
      if (currentScale > 1.05) {
        e.preventDefault();
        isZoomDragging = true;
        dragStartPos = {
          x: e.clientX - currentTx,
          y: e.clientY - currentTy
        };
      }
    });

    window.addEventListener('mousemove', (e) => {
      if (isZoomDragging && currentScale > 1.05) {
        e.preventDefault();
        currentTx = e.clientX - dragStartPos.x;
        currentTy = e.clientY - dragStartPos.y;
        clampZoomBounds();
        applyZoomTransform(false);
      }
    });
  ...
  function openGallery(images, startIndex, name) {
    ...
    currentGalleryImages = imgArray.map(img => extractImageUrl(img)).filter(Boolean);
  ...
  function handleSwipe() {
    if (typeof currentScale !== 'undefined' && currentScale > 1.05) return; // Ignore swipe navigation when zoomed in
  ...
  // Initialize Zoom Gestures
  initZoomGestureHandlers();
